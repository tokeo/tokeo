...
what it may say. Rules come from the guard's config entry (``allow`` and
...
class TokeoAiPolicyGuard(TokeoAiGuard):
    ...
    The rules are read from ``_meta`` (set from the guard's config entry):
    ``deny`` is a denylist and always wins; ``allow``, when set, is an
    allowlist that restricts calls to its members. With neither rule the
    guard permits every call (it then only documents intent).

    """

    class Meta:
        """Policy rules, overridden per guard by its config entry."""
